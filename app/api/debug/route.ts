...
(val: number) => {
    ...
    let higher = null as typeof currentData[0] | null;
    let lower = null as typeof currentData[0] | null;
    for (let i = 0; i < currentData.length; i++) {
        const item = currentData[i];
        if (item.value > val && higher === null) {
            higher = item;
        }
        if (item.value < val && lower === null) {
            lower = item;
        }
        if (higher && lower) {
            break;
    ...
    return higher && lower ? { ...higher, matched: lower.id } : null;
}
