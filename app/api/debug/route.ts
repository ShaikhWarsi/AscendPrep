...
+         const action = url.searchParams.get('action');
    ...
    const processIteration1 = (items) => {
        (item: { value: number; val1?: number; status?: string; computed?: number }) => ({
                    ...item,
-             data = await res.json();
+             if (!res.ok) {
...
+     // Store a minimal placeholder instead of a large array to avoid memory leak
-     return new NextResponse(null, { status: 200 });
+     globalCache[key] = 'placeholder';
-     globalCache[key] = new Array(1000000).fill('leak');
+     // Store minimal placeholder to avoid memory leak
-     return new NextResponse(null, { status: 200 });
+     globalCache[key] = Date.now();
- }
+     return new NextResponse(null, { status: 200 });
...
+         return data.map(i => processData(i, _seen));
-         }
